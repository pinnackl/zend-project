...
class TranslationController extends AbstractActionController
{
    ...
    public function indexAction()
	{
		...
		$dql = "SELECT a, u, l, c, h  FROM Cms\Entity\Article a LEFT JOIN a.author u LEFT JOIN a.language l LEFT JOIN a.categories c LEFT JOIN a.children h WHERE a.artcId = ?1";
...
//			$post->artcCreated = $post->artcCreatedDate . ' ' . $post->artcCreatedTime;
    ...
    public function editAction()
	{
        ...
        if (!$id) return $this->redirect()->toRoute('cms/default', array('controller' => 'translation', 'action' => 'add'), true);
...
//				$this->prepareData($article);
				$entityManager->persist($article);
				$entityManager->flush();
                return $this->redirect()->toRoute('cms/default', array('controller' => 'translation', 'action' => 'index'), true);
			 }
		}
    ...
    public function deleteAction()
	{
        ...
        $id = (int) $this->params()->fromRoute('id2', 0);
        if (!$id) return $this->redirect()->toRoute('cms/default', array('controller' => 'translation', 'action' => 'index'), true);
